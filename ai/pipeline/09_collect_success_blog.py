"""
09_collect_success_blog.py — 네이버 블로그 성공 사례 크롤링 (AI-01)

08번(지식iN)과 동일한 패턴을 네이버 블로그 검색 API로 적용.

산출물:
- ai/data/success_blog_candidates.csv
- ai/data/success_blog_strict.csv
- ai/data/success_blog_review.csv

실행:
  cd ~/Sidepick/ai
  venv/bin/python pipeline/09_collect_success_blog.py
...
AD_KEYWORDS = [
    ...
    "유료 클래스", "VIP", "단톡방",
    # 블로그 특유의 광고 키워드 추가
    "광고", "공동구매", "협찬", "할인 코드", "쿠폰", "프로모션",
    "구매 링크", "원데이 클래스", "수강 후기 이벤트",
...
def search_blog(query, start=1, display=100):
    """네이버 블로그 검색 API"""
...
for keyword in KEYWORDS:
    ...
    for start in range(1, MAX_START, DISPLAY):
        try:
            data = search_blog(keyword, start=start, display=DISPLAY)
...
fieldnames = [
    "keyword", "category", "title", "description", "link", "bloggername", "postdate",
...
out_all = DATA_DIR / "success_blog_candidates.csv"
out_strict = DATA_DIR / "success_blog_strict.csv"
out_review = DATA_DIR / "success_blog_review.csv"
