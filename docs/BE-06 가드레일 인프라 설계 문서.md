...
- 분석/검색 쿼리: AI-05 명세의 `ALLOWED_CATEGORIES` 7개
- 가이드/정책 연계: `CategoryMapper` 16개 전체 매핑
...
| `content-sns` | `콘텐츠·SNS` |
| `digital-product` | `디지털·지식판매` |
| `platform-labor` | `플랫폼 노동` |
| `talent` | `재능·프리랜서` |
| `investment` | `투자·재테크` |
| `offline` | `오프라인 부업` |
| `pre-start` | `부업 시작 전 공통` |
| `tax` | `세금·사업자` |
| `work-balance` | `본업 + 부업` |
| `marketing` | `마케팅·광고 운영` |
| `tools` | `도구·툴 추천` |
...
- 검증 전후 변환은 `CategoryMapper.toSlug()`, `CategoryMapper.toKorean()`으로 통일
