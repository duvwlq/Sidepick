...
import { getExperience, getRelatedSuccessCases, type Experience } from '../lib/api';
...
function SuccessCaseCard({ experience }: { experience: Experience }) {
  ...
  const preview =
    experience.content.replace(/!\[[^\]]*]\(([^)]+)\)/g, '').replace(/\s+/g, ' ').trim() || '본문 미리보기';
    ...
    >
      ...
      <div className={`mt-[14px] flex items-start gap-[12px] ${imageMeta.primaryImageUrl ? '' : 'min-h-[72px]'}`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className={`mt-[10px] font-['Pretendard'] text-[14px] font-[400] leading-[22px] tracking-[0px] text-[#494949] ${imageMeta.primaryImageUrl ? 'line-clamp-3' : 'line-clamp-4'}`}>
            {preview}
          </p>
        </div>
      </div>
