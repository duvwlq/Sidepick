...
function splitByHeaders(answer: string): Record<string, string> {
  ...
  for (let index = 0; index < matches.length; index += 1) {
    ...
    const bodyStart = matches[index].end;
    const bodyEnd = next ? next.start : answer.length;
    result[matches[index].header] = answer.slice(bodyStart, bodyEnd).trim();
  }
...
function parseAnswerSections(categoryId: string, answer: string): ParsedSections {
  ...
  const isBusiness =
    ...
    && TAG_DEFINITIONS.slice(1, BUSINESS_CATEGORY_COUNT + 1).some((tag) => tag.id === categoryId);
  ...
  return (
    <div className="flex flex-col gap-[8px] bg-[#F8F8F8] p-[16px]">
      <div className="rounded-[4px] bg-white p-[12px] shadow-[0_0_2px_rgba(0,0,0,0.1)]">
        ...
        <div className="flex flex-col gap-[8px] pt-[8px]">
          {steps.map((entry, index) => (
            <div key={`${category.id}-${index}`} className="flex items-start gap-[6px]">
              ...
              <p className="pt-[1px] font-['Pretendard'] text-[12px] font-[500] leading-[16.8px] tracking-[0px] text-[#131416]">
                {entry}
              </p>
            </div>
          ))}
      ...
      <div className="rounded-[4px] bg-white p-[12px] shadow-[0_0_2px_rgba(0,0,0,0.1)]">
        ...
        <p className="pt-[12px] font-['Pretendard'] text-[13px] font-[400] leading-[20px] tracking-[0px] text-[#131416]">
          {answer}
        </p>
      </div>
