...
import arrowLeftIcon from '../assets/explore-v3-figma-icons/?¨Î? ?êÏÉâ v.2 - Í≤Ä?âÏñ¥Î•?ÏπòÍ≥† ?§Ïñ¥??Í≤ΩÏö∞?êÎßå ?†ÏÇ¨???úÏãú/Arrow left.svg';
import chevronDownIcon from '../assets/explore-v3-figma-icons/?¨Î? ?êÏÉâ v.2 - Í≤Ä?âÏñ¥Î•?ÏπòÍ≥† ?§Ïñ¥??Í≤ΩÏö∞?êÎßå ?†ÏÇ¨???úÏãú/Chevron down.svg';
import filterIcon from '../assets/explore-v3-figma-icons/?¨Î? ?êÏÉâ v.2 - Í≤Ä?âÏñ¥Î•?ÏπòÍ≥† ?§Ïñ¥??Í≤ΩÏö∞?êÎßå ?†ÏÇ¨???úÏãú/tune_24dp_1F1F1F_FILL0_wght400_GRAD0_opsz24 1.svg';
import helpIcon from '../assets/explore-figma/help.svg';
import bookmarkIcon from '../assets/explore-v3-figma-icons/?¨Î? ?êÏÉâ v.2 - Í≤Ä?âÏñ¥Î•?ÏπòÍ≥† ?§Ïñ¥??Í≤ΩÏö∞?êÎßå ?†ÏÇ¨???úÏãú/Bookmark.svg';
import guideIcon from '../assets/explore-v3-figma-icons/?¨Î? ?êÏÉâ v.2 - Í≤Ä?âÏñ¥Î•?ÏπòÍ≥† ?§Ïñ¥??Í≤ΩÏö∞?êÎßå ?†ÏÇ¨???úÏãú/NavigationBar/live_help_20dp_1F1F1F_FILL0_wght400_GRAD0_opsz20 1.svg';
import heartIcon from '../assets/explore-v3-figma-icons/?¨Î? ?êÏÉâ v.2 - Í≤Ä?âÏñ¥Î•?ÏπòÍ≥† ?§Ïñ¥??Í≤ΩÏö∞?êÎßå ?†ÏÇ¨???úÏãú/Heart.svg';
import homeIcon from '../assets/explore-v3-figma-icons/?¨Î? ?êÏÉâ v.2 - Í≤Ä?âÏñ¥Î•?ÏπòÍ≥† ?§Ïñ¥??Í≤ΩÏö∞?êÎßå ?†ÏÇ¨???úÏãú/Home.svg';
import plusIcon from '../assets/explore-v3-figma-icons/?¨Î? ?êÏÉâ v.2 - Í≤Ä?âÏñ¥Î•?ÏπòÍ≥† ?§Ïñ¥??Í≤ΩÏö∞?êÎßå ?†ÏÇ¨???úÏãú/Plus.svg';
import searchNavIcon from '../assets/explore-v3-figma-icons/?¨Î? ?êÏÉâ v.2 - Í≤Ä?âÏñ¥Î•?ÏπòÍ≥† ?§Ïñ¥??Í≤ΩÏö∞?êÎßå ?†ÏÇ¨???úÏãú/Search.svg';
import userIcon from '../assets/explore-v3-figma-icons/?¨Î? ?êÏÉâ v.2 - Í≤Ä?âÏñ¥Î•?ÏπòÍ≥† ?§Ïñ¥??Í≤ΩÏö∞?êÎßå ?†ÏÇ¨???úÏãú/User.svg';
import searchIcon from '../assets/explore-v3-figma-icons/?¨Î? ?êÏÉâ v.2 - Í≤Ä?âÏñ¥Î•?ÏπòÍ≥† ?§Ïñ¥??Í≤ΩÏö∞?êÎßå ?†ÏÇ¨???úÏãú/Search.svg';
...
const CATEGORY_OPTIONS: ExploreCategoryOption[] = [
  { id: null, label: '?ÑÏ≤¥', slug: null },
  { id: 1, label: '?®Îùº???êÎß§ ¬∑ ?¥Ïª§Î®∏Ïä§', slug: 'online-commerce' },
  { id: 2, label: 'ÏΩòÌÖêÏ∏?¬∑ SNS Í∏∞Î∞ò', slug: 'content-sns' },
  { id: 3, label: '?îÏ????ÅÌíà ¬∑ ÏßÄ???êÎß§', slug: 'digital-products' },
  { id: 4, label: '?åÎû´??Í∏∞Î∞ò ?∏Îèô??, slug: 'platform-labor' },
  { id: 5, label: '?¨Îä• ?êÎß§ ¬∑ ?ÑÎ¶¨?úÏÑú', slug: 'talent-freelance' },
  { id: 6, label: '?¨Ïûê ¬∑ ?¨ÌÖå??, slug: 'investment' },
  { id: 7, label: '?§ÌîÑ?ºÏù∏ Í∏∞Î∞ò Î∂Ä??, slug: 'offline-sidejob' },
];

const SORT_OPTIONS: Array<{ key: SortKey; label: string }> = [
  { key: 'latest', label: 'ÏµúÏã†?? },
  { key: 'likes', label: 'Ï∂îÏ≤ú?? },
  { key: 'views', label: 'Ï°∞Ìöå?òÏàú' },
];

const FEED_OPTIONS: Array<{ key: FeedMode; label: string }> = [
  { key: 'all', label: '?ÑÏ≤¥' },
  { key: 'failure', label: '?§Ìå®' },
  { key: 'success', label: '?±Í≥µ' },
];
    ...
    <section className="w-[375px] bg-white pb-[4px]">
      ...
      {open ? (
        <div className="flex flex-col items-start gap-[12px] px-[16px] pb-[12px] pt-[8px]">
          ...
          {!loading && !error && !patternStats ? <PageMessage message="?µÍ≥Ñ ?∞Ïù¥?∞Î? Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??" /> : null}
              ...
              <article className="flex w-[343px] flex-col gap-[12px] rounded-[8px] border border-[#EEEEEE] bg-white px-[16px] py-[16px]">
                <div className="flex flex-col gap-[4px]">
                  ...
                  <p
                    ...
                    style={textFeatureStyle}
                  >
                    Ïπ¥ÌÖåÍ≥†Î¶¨ ??Í∞Ä??ÎßéÏù¥ ?òÌ??òÎäî ?§Ìå® ?êÏù∏?ÖÎãà??
                  </p>
                </div>
              ...
              <article className="flex w-[343px] flex-col gap-[12px] rounded-[8px] border border-[#EEEEEE] bg-white px-[16px] py-[16px]">
                <div className="flex flex-col gap-[4px]">
                  ...
                  <p
                    ...
                    style={textFeatureStyle}
                  >
                    Ïπ¥ÌÖåÍ≥†Î¶¨ ???òÌ??òÎäî ?§Ìå® ?®ÌÑ¥ ÎπÑÏ§ë Í∑∏Îûò???∞Ïù¥?∞ÏûÖ?àÎã§.
                  </p>
                </div>
              ...
              <article className="flex w-[343px] flex-col gap-[12px] rounded-[8px] border border-[#EEEEEE] bg-white px-[16px] py-[16px]">
                <div className="flex flex-col gap-[4px]">
                  ...
                  <p
                    ...
                    style={textFeatureStyle}
                  >
                    Ïπ¥ÌÖåÍ≥†Î¶¨ ???§Ìå®Î•?Í≤™Îäî ?úÏ†ê??Î∂ÑÌè¨ Í∑∏Îûò?ÑÏûÖ?àÎã§.
                  </p>
                </div>
                ...
                <div className="relative h-[188px] w-[311px]">
                  ...
                  <div className="absolute left-[106px] top-[24px] flex min-h-[40px] min-w-[141px] items-center justify-center rounded-[12px] border border-[#5A876E] bg-white px-[10px]">
                    <span
                      ...
                      style={textFeatureStyle}
                    >
                      {`${timingChart.peakMonth}Í∞úÏõî Ï∞®Ïóê Í∞Ä??ÎßéÏù¥ Î∞úÏÉù`}
                    </span>
                  </div>
...
function ReviewCardRow({
  ...
  const primaryLabel = experience.caseStatus === 'SUCCESS' ? '?±Í≥µ' : '?§Ìå®';
  const categoryLabel = resolveCategoryLabel(experience);
        ...
        <div className={`flex w-full flex-col items-start ${fullVariant ? 'gap-[6px]' : 'gap-[8px]'}`}>
          <div className="flex w-full items-center justify-between">
            <div
              ...
              style={textFeatureStyle}
            >
              <span className="truncate">{experience.author.nickname || '?âÎÑ§??}</span>
              <span>??/span>
...
function FooterArea({
  ...
  const navItems = [
    ...
    { key: 'guide', label: 'Í∞Ä?¥Îìú', icon: guideIcon, active: location.pathname === '/faq' || location.pathname === '/mypage/faq', path: '/faq' },
      ...
      accessory={
        <>
          ...
          <div className="relative flex h-[36px] w-[36px] shrink-0 items-center justify-center">
            ...
            <button
              type="button"
              aria-label={fabOpen ? 'Í≤ΩÌóò ?ëÏÑ± Î©îÎâ¥ ?´Í∏∞' : 'Í≤ΩÌóò ?ëÏÑ± Î©îÎâ¥ ?¥Í∏∞'}
...
export default function ExploreV3() {
  ...
  useEffect(() => {
    ...
    async function loadExperiences() {
      ...
      try {
        ...
        setListError(resolveErrorMessage(error, '?¨Î? Î™©Î°ù??Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??'));
      } finally {
        if (active) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .catch((error) => {
        ...
        setStatsError(resolveErrorMessage(error, 'Ïπ¥ÌÖåÍ≥†Î¶¨ ?µÍ≥ÑÎ•?Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??'));
  ...
  async function handleBookmarkToggle(experience: Experience) {
    if (!accessToken) {
      navigate(
        `/auth?next=${encodeURIComponent('/v3/explore')}&reason=${encodeURIComponent(
          'Î∂ÅÎßà?¨Îäî Î°úÍ∑∏?∏Ïù¥ ?ÑÏöî???úÎπÑ?§ÏûÖ?àÎã§.',
        )}`,
      );
      return;
    }
    ...
    try {
      ...
      showToast(payload.bookmarked ? 'Î∂ÅÎßà?¨Ïóê Ï∂îÍ??àÏñ¥??' : 'Î∂ÅÎßà?¨Î? ?¥Ï†ú?àÏñ¥??');
    } catch (error) {
      showToast(resolveErrorMessage(error, 'Î∂ÅÎßà??Ï≤òÎ¶¨???§Ìå®?àÏäµ?àÎã§.'));
    }
  }
  ...
  async function handleHeartToggle(experience: Experience) {
    if (!accessToken) {
      navigate(
        `/auth?next=${encodeURIComponent('/v3/explore')}&reason=${encodeURIComponent(
          'Î∞òÏùë Í∏∞Îä•?Ä Î°úÍ∑∏?∏Ïù¥ ?ÑÏöî???úÎπÑ?§ÏûÖ?àÎã§.',
        )}`,
      );
      return;
    }
    ...
    try {
      ...
      showToast(resolveErrorMessage(error, 'Î∞òÏùë Ï≤òÎ¶¨???§Ìå®?àÏäµ?àÎã§.'));
    }
  }
  ...
  async function handleCardCta(experience: Experience) {
    ...
    try {
      ...
      if (!target) {
        showToast('?∞Í≤∞???±Í≥µ ?¨Î?Í∞Ä ?ÑÏßÅ ?ÜÏñ¥??');
        return;
      }
      navigate(`/experiences/${target.id}`);
    } catch (error) {
      showToast(resolveErrorMessage(error, '?±Í≥µ ?¨Î?Î•?Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??'));
    }
  }
    ...
    if (!accessToken) {
      navigate(
        `/auth?next=${encodeURIComponent('/create')}&reason=${encodeURIComponent(
          'Í≤ΩÌóò ?ëÏÑ±?Ä Î°úÍ∑∏?∏Ïù¥ ?ÑÏöî???úÎπÑ?§ÏûÖ?àÎã§.',
        )}`,
      );
      return;
    }
