...
import arrowLeftIcon from '../assets/explore-v3-figma-icons/?¬ë? ?ìƒ‰ v.2 - ê²€?‰ì–´ë¥?ì¹˜ê³  ?¤ì–´??ê²½ìš°?ë§Œ ? ì‚¬???œì‹œ/Arrow left.svg';
import chevronDownIcon from '../assets/explore-v3-figma-icons/?¬ë? ?ìƒ‰ v.2 - ê²€?‰ì–´ë¥?ì¹˜ê³  ?¤ì–´??ê²½ìš°?ë§Œ ? ì‚¬???œì‹œ/Chevron down.svg';
import filterIcon from '../assets/explore-v3-figma-icons/?¬ë? ?ìƒ‰ v.2 - ê²€?‰ì–´ë¥?ì¹˜ê³  ?¤ì–´??ê²½ìš°?ë§Œ ? ì‚¬???œì‹œ/tune_24dp_1F1F1F_FILL0_wght400_GRAD0_opsz24 1.svg';
import helpIcon from '../assets/explore-figma/help.svg';
import bookmarkIcon from '../assets/explore-v3-figma-icons/?¬ë? ?ìƒ‰ v.2 - ê²€?‰ì–´ë¥?ì¹˜ê³  ?¤ì–´??ê²½ìš°?ë§Œ ? ì‚¬???œì‹œ/Bookmark.svg';
import guideIcon from '../assets/explore-v3-figma-icons/?¬ë? ?ìƒ‰ v.2 - ê²€?‰ì–´ë¥?ì¹˜ê³  ?¤ì–´??ê²½ìš°?ë§Œ ? ì‚¬???œì‹œ/NavigationBar/live_help_20dp_1F1F1F_FILL0_wght400_GRAD0_opsz20 1.svg';
import heartIcon from '../assets/explore-v3-figma-icons/?¬ë? ?ìƒ‰ v.2 - ê²€?‰ì–´ë¥?ì¹˜ê³  ?¤ì–´??ê²½ìš°?ë§Œ ? ì‚¬???œì‹œ/Heart.svg';
import homeIcon from '../assets/explore-v3-figma-icons/?¬ë? ?ìƒ‰ v.2 - ê²€?‰ì–´ë¥?ì¹˜ê³  ?¤ì–´??ê²½ìš°?ë§Œ ? ì‚¬???œì‹œ/Home.svg';
import plusIcon from '../assets/explore-v3-figma-icons/?¬ë? ?ìƒ‰ v.2 - ê²€?‰ì–´ë¥?ì¹˜ê³  ?¤ì–´??ê²½ìš°?ë§Œ ? ì‚¬???œì‹œ/Plus.svg';
import searchNavIcon from '../assets/explore-v3-figma-icons/?¬ë? ?ìƒ‰ v.2 - ê²€?‰ì–´ë¥?ì¹˜ê³  ?¤ì–´??ê²½ìš°?ë§Œ ? ì‚¬???œì‹œ/Search.svg';
import userIcon from '../assets/explore-v3-figma-icons/?¬ë? ?ìƒ‰ v.2 - ê²€?‰ì–´ë¥?ì¹˜ê³  ?¤ì–´??ê²½ìš°?ë§Œ ? ì‚¬???œì‹œ/User.svg';
import searchIcon from '../assets/explore-v3-figma-icons/?¬ë? ?ìƒ‰ v.2 - ê²€?‰ì–´ë¥?ì¹˜ê³  ?¤ì–´??ê²½ìš°?ë§Œ ? ì‚¬???œì‹œ/Search.svg';
...
const CATEGORY_OPTIONS: ExploreCategoryOption[] = [
  { id: null, label: '?„ì²´', slug: null },
  { id: 1, label: '?¨ë¼???ë§¤ Â· ?´ì»¤ë¨¸ìŠ¤', slug: 'online-commerce' },
  { id: 2, label: 'ì½˜í…ì¸?Â· SNS ê¸°ë°˜', slug: 'content-sns' },
  { id: 3, label: '?”ì????í’ˆ Â· ì§€???ë§¤', slug: 'digital-products' },
  { id: 4, label: '?Œë«??ê¸°ë°˜ ?¸ë™??, slug: 'platform-labor' },
  { id: 5, label: '?¬ëŠ¥ ?ë§¤ Â· ?„ë¦¬?œì„œ', slug: 'talent-freelance' },
  { id: 6, label: '?¬ì Â· ?¬í…Œ??, slug: 'investment' },
  { id: 7, label: '?¤í”„?¼ì¸ ê¸°ë°˜ ë¶€??, slug: 'offline-sidejob' },
];

const SORT_OPTIONS: Array<{ key: SortKey; label: string }> = [
  { key: 'latest', label: 'ìµœì‹ ?? },
  { key: 'likes', label: 'ì¶”ì²œ?? },
  { key: 'views', label: 'ì¡°íšŒ?˜ìˆœ' },
];

const FEED_OPTIONS: Array<{ key: FeedMode; label: string }> = [
  { key: 'all', label: '?„ì²´' },
  { key: 'failure', label: '?¤íŒ¨' },
    ...
    <section className="w-[375px] bg-white pb-[4px]">
      ...
      {open ? (
        <div className="flex flex-col items-start gap-[12px] px-[16px] pb-[12px] pt-[8px]">
          ...
          {!loading && !error && !patternStats ? <PageMessage message="?µê³„ ?°ì´?°ë? ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??" /> : null}
              ...
              <article className="flex w-[343px] flex-col gap-[12px] rounded-[8px] border border-[#EEEEEE] bg-white px-[16px] py-[16px]">
                <div className="flex flex-col gap-[4px]">
                  ...
                  <p
                    ...
                    style={textFeatureStyle}
                  >
                    ì¹´í…Œê³ ë¦¬ ???˜í??˜ëŠ” ?¤íŒ¨ ?¨í„´ ë¹„ì¤‘ ê·¸ë˜???°ì´?°ì…?ˆë‹¤.
                  </p>
                </div>
              ...
              <article className="flex w-[343px] flex-col gap-[12px] rounded-[8px] border border-[#EEEEEE] bg-white px-[16px] py-[16px]">
                <div className="flex flex-col gap-[4px]">
                  ...
                  <p
                    ...
                    style={textFeatureStyle}
                  >
                    ì¹´í…Œê³ ë¦¬ ???¤íŒ¨ë¥?ê²ªëŠ” ?œì ??ë¶„í¬ ê·¸ë˜?„ì…?ˆë‹¤.
                  </p>
                </div>
                ...
                <div className="relative h-[188px] w-[311px]">
                  ...
                  <div className="absolute left-[106px] top-[24px] flex min-h-[40px] min-w-[141px] items-center justify-center rounded-[12px] border border-[#5A876E] bg-white px-[10px]">
                    <span
                      ...
                      style={textFeatureStyle}
                    >
                      {`${timingChart.peakMonth}ê°œì›” ì°¨ì— ê°€??ë§ì´ ë°œìƒ`}
                    </span>
                  </div>
...
function ReviewCardRow({
  ...
  const primaryLabel = experience.caseStatus === 'SUCCESS' ? '?±ê³µ' : '?¤íŒ¨';
  const categoryLabel = resolveCategoryLabel(experience);
        ...
        <div className={`flex w-full flex-col items-start ${fullVariant ? 'gap-[6px]' : 'gap-[8px]'}`}>
          <div className="flex w-full items-center justify-between">
            <div
              ...
              style={textFeatureStyle}
            >
              <span className="truncate">{experience.author.nickname || '?‰ë„¤??}</span>
              <span>??/span>
...
function FooterArea({
  ...
  const navItems = [
    ...
    { key: 'guide', label: 'ê°€?´ë“œ', icon: guideIcon, active: location.pathname === '/faq' || location.pathname === '/mypage/faq', path: '/faq' },
      ...
      accessory={
        <>
          ...
          <div className="relative flex h-[36px] w-[36px] shrink-0 items-center justify-center">
            ...
            <button
              type="button"
              aria-label={fabOpen ? 'ê²½í—˜ ?‘ì„± ë©”ë‰´ ?«ê¸°' : 'ê²½í—˜ ?‘ì„± ë©”ë‰´ ?´ê¸°'}
...
export default function ExploreV3() {
  ...
  useEffect(() => {
    ...
    async function loadExperiences() {
      ...
      try {
        ...
        setListError(resolveErrorMessage(error, '?¬ë? ëª©ë¡??ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??'));
      } finally {
        if (active) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .catch((error) => {
        ...
        setStatsError(resolveErrorMessage(error, 'ì¹´í…Œê³ ë¦¬ ?µê³„ë¥?ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??'));
  ...
  async function handleBookmarkToggle(experience: Experience) {
    ...
    try {
      ...
      showToast(payload.bookmarked ? 'ë¶ë§ˆ?¬ì— ì¶”ê??ˆì–´??' : 'ë¶ë§ˆ?¬ë? ?´ì œ?ˆì–´??');
    } catch (error) {
      showToast(resolveErrorMessage(error, 'ë¶ë§ˆ??ì²˜ë¦¬???¤íŒ¨?ˆìŠµ?ˆë‹¤.'));
    }
  }
  ...
  async function handleHeartToggle(experience: Experience) {
    ...
    try {
      ...
      showToast(resolveErrorMessage(error, 'ë°˜ì‘ ì²˜ë¦¬???¤íŒ¨?ˆìŠµ?ˆë‹¤.'));
    }
  }
  ...
  async function handleCardCta(experience: Experience) {
    ...
    try {
      ...
      if (!target) {
        showToast('?°ê²°???±ê³µ ?¬ë?ê°€ ?„ì§ ?†ì–´??');
        return;
      }
      navigate(`/experiences/${target.id}`);
    } catch (error) {
      showToast(resolveErrorMessage(error, '?±ê³µ ?¬ë?ë¥?ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??'));
    }
  }
