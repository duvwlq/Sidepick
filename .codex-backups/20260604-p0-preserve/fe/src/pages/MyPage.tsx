...
import { BOOKMARK_SYNC_EVENT, publishBookmarkSync } from '../lib/bookmark-sync';
...
export default function MyPage() {
  ...
  useEffect(() => {
    ...
    function handleBookmarkSync() {
      void reloadBookmarks();
    }

    window.addEventListener(BOOKMARK_SYNC_EVENT, handleBookmarkSync);
    return () => {
      cancelled = true;
      window.removeEventListener(BOOKMARK_SYNC_EVENT, handleBookmarkSync);
    };
  }, [token]);
  ...
  async function handleBookmarkRemove(cardId: number) {
    ...
    try {
      const payload = await unbookmarkExperience(token, cardId);
