...
public class AgentAService {
    ...
    private static final String FALLBACK_MESSAGE =
            "질문 카드를 생성하지 못했어요. 그대로 진행하거나 잠시 후 다시 시도해 주세요.";
    ...
    private static final List<QuestionTemplate> QUESTION_TEMPLATES = List.of(
            new QuestionTemplate("goal", "이번 글에서 얻고 싶은 결과가 무엇인가요?", "text", null, true, "예: 실패 원인 분석, 다음 방향 정리"),
            new QuestionTemplate("timeline", "얼마 동안 시도했는지 알려주세요.", "select",
                    List.of("1개월 미만", "1~3개월", "3~6개월", "6개월 이상"), true, null),
            new QuestionTemplate("budget", "시작할 때 들어간 비용은 어느 정도였나요?", "number", null, false,
                    "대략적인 총액만 적어도 충분합니다."),
            ...
            new QuestionTemplate("obstacle", "가장 크게 막혔던 지점을 적어주세요.", "tag", null, true,
                    "여러 개라면 대표 2~3개만 적어주세요."),
            new QuestionTemplate("market", "시장 조사나 경쟁 분석은 어느 정도 했나요?", "select",
                    List.of("충분히 했다", "간단히 했다", "거의 못 했다"), false, null),
            new QuestionTemplate("channel", "사용했던 유입 채널을 알려주세요.", "tag", null, false,
    ...
    public AnalyzeDraftResponse analyzeDraft(AnalyzeDraftRequest request) {
        ...
        try {
            ...
            return new AnalyzeDraftResponse(
                    ...
                    needsQuestions ? null : "지금 초안은 추가 질문 없이도 진행 가능한 상태예요."
            );
